...
- 代码来源：local
- 识别版本：3 / 3
- 风险概况：Blocker 0，High 4，Medium 2，Low 2
...
- 生成时间：2026-07-10T23:58:32+08:00
...
- Java：C:\Program Files\Common Files\Oracle\Java\javapath\java.EXE；Git：F:\Git\mingw64\bin\git.EXE；Playwright：missing

### 代码来源

| 来源 | 状态 | 结果码 | 仓库 / 路径 | Markdown 位置 | Ref | Commit | 抓取时间 | 历史深度 | 说明 |
|---|---|---|---|---|---|---|---|---:|---|
| local | PASS | LOCAL_WORKTREE_DIRTY | F:\学校活动\双创周\mission5 | - | - | 660e9d589648 | - | 6 | 工作区非洁净（7 个变更或未跟踪条目）；Commit 仅表示仓库基线 |
...
|---|---:|---:|---:|---|
| 整体评分 | 39.52 | 34.00 | 40 | - |
| springboot-vue-version | 19.07 | 14.65 | 20 | Spring Boot + Vue |
| django-version | 18.73 | 13.90 | 20 | Django |
| mern-version | 20.00 | 15.00 | 20 | MERN |
| **总计** | **97.32** | **77.55** | **100** | **NOT_VERIFIED** |

### 整体评分项

| 评分项 | 静态暂定 | 证据确认 | 满分 | 状态 | 主要缺口 |
|---|---:|---:|---:|---|---|
| 应用概念与最低功能范围 | 9.52 | 4.00 | 10 | PARTIAL | 无 |
...
### D5-003 [High] Spring 业务错误仍可能返回 HTTP 200
...
### D5-004 [High] Spring 验证注解未绑定到可验证对象
...
### D5-008 [Low] 本地提交快照未固定到洁净 Git 状态

- 范围：`submission`
- 状态：`FAIL_PROJECT`
- 期望：评测输入可由明确 commit 或归档文件确定性复现
- 实际：工作区非洁净（7 个变更或未跟踪条目）；Commit 仅表示仓库基线
...
- 核心资源：`/api/prompts`
- 分数：暂定 19.07/20，确认 14.65/20

| 评分项 | 暂定 | 确认 | 满分 | 置信度 | 缺口 |
|---|---:|---:|---:|---|---|
| 源代码完整性 | 5.00 | 4.00 | 5 | high | 无 |
| README 文档 | 5.00 | 4.00 | 5 | high | 无 |
| 应用功能 | 4.07 | 1.65 | 5 | medium | 未证明服务端路径会触发验证；缺少异常处理 |
...
| 错误处理与统一响应 | 0.27 | 0.10 | 0.8 | PARTIAL |
...
- 核心资源：`/api/prompts`
- 分数：暂定 20.00/20，确认 15.00/20

| 评分项 | 暂定 | 确认 | 满分 | 置信度 | 缺口 |
|---|---:|---:|---:|---|---|
| 源代码完整性 | 5.00 | 4.00 | 5 | high | 无 |
| README 文档 | 5.00 | 4.00 | 5 | high | 无 |
| 应用功能 | 5.00 | 2.00 | 5 | high | 无 |
| writeup 描述 | 5.00 | 5.00 | 5 | high | 无 |
...
|---|---|---|---|---|---:|---|
| - | - | 未执行动态测试 | - | NOT_RUN | - | 使用 --mode dynamic 显式启用 |

## 优先行动项

1. [High] django-version：在 API 层捕获 Http404 或显式查询并调用 _json_error(..., 404)。
2. [High] django-version：补充技术栈标准依赖文件并锁定版本。
3. [High] springboot-vue-version：使用 ResponseEntity 或 @RestControllerAdvice 映射 400/404。
4. [High] springboot-vue-version：引入请求 DTO，或在 applyFields 后显式调用 Validator，并统一返回 400。
5. [Medium] django-version：补充最小单元测试和 CRUD 集成测试，并让默认测试命令可通过。
6. [Medium] mern-version：补充最小单元测试和 CRUD 集成测试，并让默认测试命令可通过。
7. [Low] mern-version：从 package.json 生成或同步技术版本说明。
8. [Low] submission：提交或归档最终文件，并在报告中记录对应 commit SHA。
